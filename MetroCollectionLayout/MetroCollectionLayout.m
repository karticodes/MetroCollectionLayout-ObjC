...
NSString *const MetroCollectionViewHeader = @"MetroCollectionViewHeader";
NSString *const MetroCollectionViewFooter = @"MetroCollectionViewFooter";
...
- (void)calculateValues{
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(wideCellWidthForCollectionView:andLayout:)]) {
        ...
        _wideCellWidth = (CGRectGetWidth(self.collectionView.bounds)/2);
    }
                ...
                if (itemsCount == 1) {
                    ...
                    if (!CGSizeEqualToSize(size, CGSizeZero)) {
                        ...
                        UICollectionViewLayoutAttributes *footerAttributes = [UICollectionViewLayoutAttributes
                                                                              layoutAttributesForSupplementaryViewOfKind:MetroCollectionViewFooter
                                                                              withIndexPath:indexPath];
...
{
    CGRect frame = CGRectZero;
    frame.origin.y =  [self getYAtIndexPath:indexPath] + [self getHeightForItemAtIndexPath:indexPath];
    frame.size = size;
...
- (CGRect)frameForCellAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([self isWideElementAtIndexPath:indexPath]) {
        ...
        CGFloat yValue = (currentGroup - 1) * self.wideCellHeight + [self heightForHeaderAtIndexPath:indexPathFirstElementCurrentSection];
        ...
        frame = CGRectMake([self getXAtIndexPath:indexPath], [self getYAtIndexPath:indexPath], self.cellWidth, self.cellHeight);
    }
...
- (CGFloat)getYAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSInteger multiplyValue;
    NSInteger maxElement = self.numberOfItems * currentGroup;
    ...
    yValue = ((currentGroup - 1) * self.wideCellHeight) + (self.cellHeight * multiplyValue) + [self heightForHeaderAtIndexPath:indexPathFirstElementCurrentSection];
...
- (NSInteger)currentGroupAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSInteger resultValue = item / self.numberOfItems;
...
- (BOOL)isWideElementAtIndexPath:(NSIndexPath *)indexPath
{
    if (self.shouldAutoAlign) {
        if ([self isFlatGroupAtIndexPath:indexPath]) {
            return NO;
        } else {
            return [self isWideElementWithAlignmentAtIndexPath:indexPath];
        }
    } else {
        return [self isWideElementWithAlignmentAtIndexPath:indexPath];
    }
...
- (BOOL)isWideElementWithAlignmentAtIndexPath:(NSIndexPath *)indexPath{
    if ([self getDirectionForGroup:[self currentGroupAtIndexPath:indexPath] inSection:indexPath.section] == MetroCollectionLayoutDirectionLeft) {
